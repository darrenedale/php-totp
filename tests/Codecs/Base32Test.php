...
final class Base32Test extends TestCase
{
    ...
    /** Data provider with simple raw content strings for testConstructor1() and testSetRaw1() */
    public static function providerSimpleRawStrings(): iterable
	{
        yield "ascii" => ["test-data-to-encode",];
        yield "binary" => ["\xff\xfe\xfd\xfc\xfb\xfa\xf8\xf7",];
        yield "empty" => ["",];
        yield "whitespace" => ["  ",];
	}

    /** Ensure we can construct a Base32 codec instance with some raw data. */
    #[DataProvider("providerSimpleRawStrings")]
    public function testConstructor1(string $plainData)
	{
        $actual = new Base32($plainData);
        self::assertSame($plainData, $actual->raw());
    }

    /** Data provider with raw string content for testDestructor1(). */
    public static function providerTestDestructor1(): iterable
    {
        yield "typical-ascii-raw" => ["password-password"];
        yield "null-bytes-16-raw" => ["\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"];
        yield "null-bytes-20-raw" => ["\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"];
        yield "null-bytes-40-raw" => [str_repeat("\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00", 2)];
        yield "null-bytes-60-raw" => [str_repeat("\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00", 3)];
        yield "null-bytes-80-raw" => [str_repeat("\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00", 4)];
        yield "null-bytes-100-raw" => [str_repeat("\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00", 5)];
        yield "null-bytes-120-raw" => [str_repeat("\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00", 6)];
        yield "null-bytes-140-raw" => [str_repeat("\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00", 7)];
        yield "null-bytes-160-raw" => [str_repeat("\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00", 8)];
        yield "null-bytes-180-raw" => [str_repeat("\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00", 9)];
        yield "null-bytes-200-raw" => [str_repeat("\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00", 10)];
    }

    /** Ensure the destructor scrubs the raw and encoded properties. */
    #[DataProvider("providerTestDestructor1")]
    public function testDestructor1(string $rawData): void
    {
        ...
        $base32->__destruct();
        self::assertAllCharactersHaveChanged($rawData, $xray->rawData, "The raw data was not overwritten with random data.");
        self::assertAllCharactersHaveChanged($base32Data, $xray->encodedData, "The base32 data was not overwritten with random data.");
    }

    /** Ensure raw data can successfully be set on the codec. */
    #[DataProvider("providerSimpleRawStrings")]
    public function testSetRaw1(mixed $plainData)
	{
        $base32 = new Base32();
		$base32->setRaw($plainData);
        self::assertSame($plainData, $base32->raw());
	}

    /** Data provider with valid base32-encoded data for testSetEncoded1() .*/
    public static function providerTestSetEncoded1(): iterable
	{
        yield "typical-ascii-data" => ["ORSXG5BNMRQXIYJNORXS2ZLOMNXWIZI=",];
        yield "typical-mixed-data" => ["ORSXG5BN74WW22LYMVSC3ABNMRQXIYJNAAWXI3ZNR4WWK3TDN5SGKCQK",];
        yield "typical-binary-data" => ["777P37H37L4PO===",];
        yield "typical-png-image-data" => [
        ...
        yield "extreme-empty-data" => ["",];
        yield "extreme-all-valid-characters" => ["ABCDEFGHIJKLMNOPQRSTUVWXYZ234567",];
	}

    /** Ensure valid base32-encoded data can be set successfully. */
    #[DataProvider("providerTestSetEncoded1")]
    public function testSetEncoded1(mixed $encodedData)
	{
		$base32Codec = new Base32();
		$base32Codec->setEncoded($encodedData);
        self::assertSame($encodedData, $base32Codec->encoded());
	}

    /** Provides "base32-encoded" strings with invalid characters for testSetEncoded2(). */
    public static function providerTestSetEncoded2(): iterable
    {
        yield "invalid-ascii-string" => ["ORSXG5BNMRQXIYJNORXS2ZLOMNXWIZi=",];
        yield "invalid-binary-data" => ["\xff\xfe\xfd\xfc\xfb\xfa\xf8\xf7",];
        yield "invalid-mixed-data" => ["ORSXG5BN74WW22LYMVSC3ABNMRQXIYJNAAWXI3ZNR4WWK3TDN5SGKCQk",];
        yield "invalid-png-data" => ["",];
    }

    /** Ensure setting base32-encoded data with invalid characters throws the expected exception. */
    #[DataProvider("providerTestSetEncoded2")]
    public function testSetEncoded2(mixed $encodedData): void
    {
        $this->expectException(InvalidBase32DataException::class);
        $this->expectExceptionMessageMatches("/^Invalid base32 character found at position \\d+$/");
        (new Base32())->setEncoded($encodedData);
    }

    /** Provides "base32-encoded" strings with invalid amounts of = padding for testSetEncoded3(). */
    public static function providerTestSetEncoded3(): iterable
    {
    ...
    /** Ensure setting base32-encoded data with incorrect padding throws the expected exception. */
    #[DataProvider("providerTestSetEncoded3")]
    public function testSetEncoded3(string $encoded): void
    {
        $this->expectException(InvalidBase32DataException::class);
    ...
    /** Provides "base32-encoded" strings with incorrect lengths for testSetEncoded4(). */
    public static function providerTestSetEncoded4(): iterable
    {
        yield "1-extra-character" => ["ORSXG5BNMRQXIYJNO"];
        yield "2-extra-characters" => ["ORSXG5BNMRQXIYJNOR"];
        yield "3-extra-characters" => ["ORSXG5BNMRQXIYJNORX"];
        yield "4-extra-characters" => ["ORSXG5BNMRQXIYJNORXS"];
        yield "5-extra-characters" => ["ORSXG5BNMRQXIYJNORXS2"];
        yield "6-extra-characters" => ["ORSXG5BNMRQXIYJNORXS2Z"];
        yield "7-extra-characters" => ["ORSXG5BNMRQXIYJNORXS2ZL"];
    }

    /** Ensure setting base32-encoded data with incorrect length throws the expected exception. */
    #[DataProvider("providerTestSetEncoded4")]
    public function testSetEncoded4(string $encoded): void
    {
        $this->expectException(InvalidBase32DataException::class);
        $this->expectExceptionMessage("Base32 data must be padded to a multiple of 8 bytes");
        (new Base32())->setEncoded($encoded);
    }

    /** Data provider with raw data and the base32-encoded equivalent for testEncoded1() and testEncode1(). */
    public static function providerRawAndEncoded(): iterable
	{
        yield "typical-ascii-data" => ["test-data-to-encode", "ORSXG5BNMRQXIYJNORXS2ZLOMNXWIZI=",];
        yield "typical-mixed-data" => ["test-\xff-mixed-\x80-data-\x00-to-\x8f-encode\n\n", "ORSXG5BN74WW22LYMVSC3ABNMRQXIYJNAAWXI3ZNR4WWK3TDN5SGKCQK",];
        yield "typical-binary-data" => ["\xff\xfe\xfd\xfc\xfb\xfa\xf8\xf7", "777P37H37L4PO===",];
        yield "typical-png-image-data" => [
    ...
    /** Ensure a codec instance correctly encodes data. */
    #[DataProvider("providerRawAndEncoded")]
    public function testEncoded1(string $plainData, string $expectedEncodedData)
	{
        self::assertSame($expectedEncodedData, (new Base32($plainData))->encoded(), "Base32::encoded() produced unexpected output");
	}

    /** Ensure the static convenience method encode() correctly encodes data. */
    #[DataProvider("providerRawAndEncoded")]
    public function testEncode1(string $plainData, string $expectedEncodedData)
    {
        self::assertSame($expectedEncodedData, Base32::encode($plainData), "static Base32::encode() produced unexpected output");
    }

    /** Data provider with valid base32-encoded strings and the decoded content for testRaw1() and testDecode1(). */
    public static function providerValidEncodedData(): iterable
	{
        yield "ascii-data" => ["ORSXG5BNMRQXIYJNORXS2ZLOMNXWIZI=", "test-data-to-encode",];
        yield "mixed-ascii-binary-data" => ["ORSXG5BN74WW22LYMVSC3ABNMRQXIYJNAAWXI3ZNR4WWK3TDN5SGKCQK", "test-\xff-mixed-\x80-data-\x00-to-\x8f-encode\n\n",];
        yield "binary-data" => ["777P37H37L4PO===", "\xff\xfe\xfd\xfc\xfb\xfa\xf8\xf7",];
        yield "png-image-data" => [
            "",
            "",
        ];
        yield "empty" => ["", "",];
    ...
    /** Ensure the codec can decode valid encoded data. */
    #[DataProvider("providerValidEncodedData")]
    public function testRaw1(string $encodedData, string $expectedRawData)
	{
		$base32Codec = new Base32();
		$base32Codec->setEncoded($encodedData);

        self::assertSame($expectedRawData, $base32Codec->raw(), "Base32::raw() produced unexpected output");
	}

    /** Ensure the static convenience method decode() can decode valid encoded data. */
    #[DataProvider("providerValidEncodedData")]
    public function testDecode1(string $encodedData, string $expectedRawData)
    {
        self::assertSame($expectedRawData, Base32::decode($encodedData), "static Base32::decode() produced unexpected output");
    }

    /** Data provider with non-strict base32-encoded strings and the decoded content for testRaw2() and testDecode2(). */
    public static function providerRelaxedValidEncodedData(): iterable
    {
        yield "ascii-without-padding" => ["ORSXG5BNMRQXIYJNORXS2ZLOMNXWIZI", "test-data-to-encode",];
    }

    /** Ensure the codec can decode "valid" encoded data in relaxed mode. */
    #[DataProvider("providerRelaxedValidEncodedData")]
    public function testRaw2(string $encodedData, string $expectedRawData)
    {
        Base32::setStrict(false);
        $base32Codec = new Base32();
        $base32Codec->setEncoded($encodedData);
    ...
    /** Ensure the static convenience method decode() can decode "valid" encoded data in relaxed mode. */
    #[DataProvider("providerRelaxedValidEncodedData")]
    public function testDecode2(string $encodedData, string $expectedRawData)
    {
        Base32::setStrict(false);
        self::assertSame($expectedRawData, Base32::decode($encodedData), "static Base32::decode() produced unexpected output");
    }
